...
1.  Compute the slope with sums only, no `lm()`:
    ...
    Then confirm both against `lm(y ~ x)`. Why is $\hat\beta_1$ not exactly $0.5$ even though the code contains the true model? One sentence.
...
where $\tilde D_i$ and $\tilde Y_i$ are residuals from regressing $D_i$ and $Y_i$ on the controls.
...
4.  **Clustered, by hand.** For the GSS bivariate model `lnearn ~ educ`, with $d_i$ the deviation of `educ` from its mean: form each sampling cluster's total $S_g = \sum_{i \in g} d_i \hat e_i$, one per `psu`, and square after summing:
