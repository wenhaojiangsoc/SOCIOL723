...
\begin{frame}{Where Projection Comes From: a Change of Picture\adv}
Every scatterplot you have drawn puts \emph{variables} on the axes and
observations as points: $n$ points in $2$ dimensions. Now flip it:
\begin{itemize}\itemsep3pt
  \item Let every \emph{observation} be an axis: $\R^n$, one dimension per
        person. The whole variable $\Y$ becomes a \emph{single arrow} in that
        space; so does each column of $\X$.
  \item As $b$ ranges over all values, the fitted vectors $\X b$ sweep out a
        $k$-dimensional plane through the origin: every fit the model could
        ever produce.
  \item Least squares asks for the point of that plane \emph{closest} to
        $\Y$. Geometry answers: drop a perpendicular. Its foot is $\hat\Y$;
        the perpendicular segment itself is $\ehat$.
  \item Perpendicular to the plane $=$ at right angles to every column of
        $\X$, that is, $\X'\ehat = \zero$. \alert{These are the normal
        equations}, and ``normal'' is the Latin word for perpendicular. The
        name was the geometry all along.
\end{itemize}
\vskip2pt
``Projection'' is simply the operation \emph{drop the perpendicular}. The next
frames give it a matrix.
\end{frame}
