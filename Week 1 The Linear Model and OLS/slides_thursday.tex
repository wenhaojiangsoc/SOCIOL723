...
\begin{itemize}\itemsep1pt
  \item \textbf{One column} (intercept only): the mixtures are all multiples
        of $\one$, a \alert{line} through the origin.
...
\begin{frame}{Where Projection Comes From: $n = 2$ First\adv}
\begin{columns}[T]
\begin{column}{0.42\textwidth}
\begin{center}
\begin{tikzpicture}[scale=0.70,>=Stealth]
  \draw[->] (0,0) -- (5.4,0);
  \node[below] at (2.7,-0.45) {\scriptsize $Y_1$ (observation 1's value)};
  \draw[->] (0,0) -- (0,5.4);
  \node[rotate=90,above] at (-0.62,2.7) {\scriptsize $Y_2$ (observation 2's value)};
...
{\small
\begin{itemize}\itemsep3pt
  \item Two observations, one variable: $Y_1 = 2$, $Y_2 = 4$. Plot the
        \emph{dataset} as one point, one axis per observation.
  \item The intercept-only model $Y_i = b + e_i$ gives every observation the
        same fitted value: the point $(b,b)$. All candidates form the
        $45^\circ$ line.
...
\end{column}
\end{columns}
{\footnotesize With $n$ observations the picture lives in $\R^n$; with regressors
