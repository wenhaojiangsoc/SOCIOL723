...
\begin{itemize}\itemsep1pt
  \item \textbf{One column} (intercept only): all multiples of the single
        arrow $\one$, a \alert{line} through the origin. One knob to turn,
        one dimension of freedom.
  \item \textbf{Two columns}: all mixtures of two arrows, a flat
        \alert{plane} through the origin. Two knobs, two dimensions.
  \item \textbf{$k$ columns}: $k$ knobs, a $k$-dimensional flat sheet inside
        $\R^n$. That is \emph{all} ``$k$-dimensional subspace'' means:
        \alert{the set of points reachable by turning $k$ independent knobs}.
  \item The data $\Y$ generally sits \emph{off} the sheet; the gap is the
        residual. And if one column is itself a mixture of the others, one
        knob is redundant and the sheet is thinner than $k$: \alert{rank},
        again.
\end{itemize}
\end{frame}

\begin{frame}{Where Projection Comes From: Two People First\adv}
\begin{columns}[T]
\begin{column}{0.42\textwidth}
\begin{center}
\begin{tikzpicture}[scale=0.70,>=Stealth]
  \draw[->] (0,0) -- (5.4,0);
  \node[below] at (2.7,-0.45) {\scriptsize person 1's value};
  \draw[->] (0,0) -- (0,5.4);
  \node[rotate=90,above] at (-0.62,2.7) {\scriptsize person 2's value};
  \foreach \t in {1,2,3,4,5} {
    \draw (\t,0) -- (\t,-0.07) node[below] {\tiny \t};
    \draw (0,\t) -- (-0.07,\t) node[left] {\tiny \t};}
  \draw[dukeblue,thick,dashed] (0,0) -- (5.15,5.15);
  \node[dukeblue,rotate=45] at (4.55,4.05) {\scriptsize all fits $(b,b)$};
  \fill (2,4) circle (2.2pt);
  \node[above left] at (2.05,4.0) {\scriptsize $\Y=(2,4)$};
  \fill[dukeblue] (3,3) circle (2.2pt);
  \node[dukeblue,below right] at (3.05,3.0) {\scriptsize $(3,3)=\bar Y\one$};
  \draw[->,accent,thick] (3,3) -- (2,4);
  \node[accent,left] at (2.5,3.42) {\scriptsize $\ehat$};
  \draw[accent] (3.18,3.18) -- (3.0,3.36) -- (2.82,3.18);
\end{tikzpicture}
\end{center}
\end{column}
...
{\small
\begin{itemize}\itemsep3pt
  \item Two people, one variable: $Y_1 = 2$, $Y_2 = 4$. Plot the
        \emph{dataset} as one point, one axis per person.
  \item The intercept-only model $Y_i = b + e_i$ fits both people the same
        number: the point $(b,b)$. All candidates form the $45^\circ$ line.
  \item Closest point on that line to $(2,4)$: the perpendicular's foot,
        $(3,3)$. So $\hat b = 3 = \bar Y$: \alert{the mean is a projection}.
  \item Right angle, checked: residual $(-1,1)$, line direction $(1,1)$,
        dot product $0$. Perpendicular $=$ residuals sum to zero $=$ (N1).
\end{itemize}}
\end{column}
\end{columns}
{\footnotesize With $n$ people the picture lives in $\R^n$; with regressors
the line becomes the plane spanned by $\X$'s columns. OLS is always the same
move: \alert{drop the perpendicular}. (``Normal'' equations: normal $=$
perpendicular.)}
\end{frame}
...
\end{columns}
\vskip2pt
{\footnotesize For the traces: $\trace(\Pm) = \trace\big((\X'\X)^{-1}\X'\X\big)
= \trace(\Ix_k) = k$, using the cyclic property $\trace(ABC)=\trace(BCA)$,
which we use repeatedly.}
\end{frame}
...
\begin{frame}{The Same Idea, Aimed at the Data: the SVD\adv}
OLS projected $\Y$ onto the closest point in $\X$'s column space. The
\alert{singular value decomposition} turns the same ``closest fit'' logic on
the data matrix itself: any $n\times k$ matrix factors as
\begin{align*}
  \X = \bm U\bm\Sigma\bm V' ,
\end{align*}
where $\bm U$ and $\bm V$ hold perpendicular \emph{directions} (through the
observations and through the variables) and $\bm\Sigma$ is diagonal with
entries $\sigma_1 \geq \sigma_2 \geq \cdots \geq 0$, the
\alert{singular values}.
\begin{itemize}\itemsep2pt
  \item Read it as: every data matrix is a set of independent
        \emph{directions of information}, ordered by how much of the data
        each carries.
  \item The rank from earlier $=$ the number of nonzero singular values.
  \item \textbf{Keep only the top $r$ directions}, and you have the rank-$r$
        matrix \emph{closest to $\X$ in the least-squares sense}: the same
        ``drop the perpendicular'' logic as OLS, applied to the whole matrix
        rather than to $\Y$.
\end{itemize}
\end{frame}

\begin{frame}{Latent Semantic Analysis: Projection Applied to Text\adv}
Let $\X$ be a \alert{document--term matrix}: one row per document, one column
per word, entries counting occurrences. Thousands of columns; far fewer
\emph{themes}.
\begin{itemize}\itemsep3pt
  \item Truncate the SVD at $r\approx100$ directions: each document becomes a
        mixture of $r$ \alert{latent semantic dimensions}, and words that
        co-occur across documents get pulled onto the same dimension. That is
        \emph{latent semantic analysis} \srcp{Deerwester et al.\ 1990}.
  \item Sociology runs on this geometry: \srct{Kozlowski, Taddy, and Evans
        (2019)} \emph{project} word vectors onto cultural axes such as
        rich--poor to track the meaning of class across a century, and the
        operation is literally the projection formula from a few frames back.
  \item Course connections: ridge regression shrinks along exactly these
        singular directions (Week 12), and synthetic control's factor model is
        a low-rank story about a panel (Week 11).
\end{itemize}
{\footnotesize None of this is examinable: it is where the geometry goes
when you point it at data sociologists care about.}
\end{frame}
...
\item Deerwester, Scott, Susan T. Dumais, George W. Furnas, Thomas K. Landauer,
and Richard Harshman. 1990. ``Indexing by Latent Semantic Analysis.''
\textit{Journal of the American Society for Information Science} 41(6):
391--407.
\item Kozlowski, Austin C., Matt Taddy, and James A. Evans. 2019. ``The Geometry
of Culture: Analyzing the Meanings of Class through Word Embeddings.''
\textit{American Sociological Review} 84(5): 905--949.
