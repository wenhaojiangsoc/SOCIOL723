...
\begin{itemize}\itemsep3pt
  ...
  \item A short matrix argument (which we do not cover) proves
        $\E[\sum_i \hat e_i^2] = \sigma^2(n-k)$ exactly; take it on trust,
        and note that it is exact, not approximate.
...
\textbf{E}stimator (BLUE). (The proof is a short matrix argument that we do
not cover.)
\end{block}
\begin{itemize}\itemsep3pt
