...
\end{enumerate}
\vskip6pt
\alert{So far, ``what is $\hat\beta_1$ estimating?'' has an answer only
because we assumed the average of $Y$ at every value of $X$ lies on a line in the \textit{population}.
Next we drop that assumption, and ask what $\hat\beta_1$ estimates when no
line is true.}
\end{frame}
...
What we have presented for estimation has been \emph{sample} algebra: we picked the
line that minimizes squared residuals, and every property we derived,
residuals summing to zero, uncorrelated with the regressors, was mechanical.
The population entered exactly \emph{once}: the assumption that the average
of $Y$ at each $X$ lies on a line, which is what defined $\beta_1$. If that
assumption is false, and the previous frame showed that it is, there is no
true $\beta_1$ for $\hat\beta_1$ to be an estimate \emph{of}.
\end{block}
\vskip3pt
\begin{itemize}\itemsep3pt
...
The last section settled \emph{what} $\hat\beta_1$ estimates. The new question
is \emph{how well}. $\hat\beta_1$ is computed from one sample; a different
sample would have produced a different number. So we ask how it behaves
\emph{across} samples:
\begin{itemize}\itemsep3pt
...
\begin{itemize}\itemsep5pt
  \item Everything so far used only scalar algebra: the least-squares recipe,
        what it estimates (the CEF and its best linear approximation),
        omitted-variable bias, and what regression can and cannot say about
        causality.
