...
\begin{frame}[shrink=10]{Notation We Will Use All Semester}
...
\begin{align*}
  ...
  \widehat\Cov(X,Y) = 0.9476, \quad \widehat\Var(X) = 7.981 .
\end{align*}
\pause
\begin{align*}
...
\begin{frame}[shrink=10]{Why This Matters}
\begin{itemize}\itemsep5pt
...
\begin{align*}
  \tilde\beta_1
  &= \frac{\widehat\Cov(X,Y)}{\widehat\Var(X)}
   = \frac{\widehat\Cov\big(X,\ \beta_0 + \beta_1X + \beta_2 W + \epsilon\big)}
          {\widehat\Var(X)} \\[4pt]
  &= \beta_1\underbrace{\frac{\widehat\Cov(X,X)}{\widehat\Var(X)}}_{=\,1}
   + \beta_2\underbrace{\frac{\widehat\Cov(X,W)}{\widehat\Var(X)}}_{=\;\delta_1}
   + \underbrace{\frac{\widehat\Cov(X,\epsilon)}{\widehat\Var(X)}}_{=\,0
...
\begin{tabular}{lcc}
\toprule
 & $\widehat\Cov(X, W) > 0$ & $\widehat\Cov(X,W) < 0$ \\
\midrule
...
\begin{frame}[shrink=10]{Omitted Variable Bias in the GSS}
...
\begin{enumerate}\itemsep4pt
  ...
  \item Derive and interpret
        $\hat\beta_1 = \widehat\Cov(X,Y)/\widehat\Var(X)$ and
...
\end{enumerate}
\vskip6pt
\alert{Everything from here on is the same content in more general clothing.}
\end{frame}
...
\begin{frame}[shrink=10]{The Classical Assumptions}
...
\begin{frame}[shrink=10]{What Drives Precision}
\begin{align*}
  \Var(\hat\beta_1\mid X) = \frac{\sigma^2}{\sum_i (X_i - \bar X)^2}
  = \frac{\sigma^2}{n\,\widehat\Var(X)} ,
\end{align*}
and with more regressors,
$\Var(\hat\beta_j) = \dfrac{\sigma^2}{n\,\widehat\Var(X_j)\,(1-R_j^2)}$
...
\begin{itemize}\itemsep3pt
  ...
  \item Then $\widehat{\Var}(\hat\beta_1) = s^2/\sum_i (X_i-\bar X)^2$, whose
...
\end{align*}
so that
$t = \dfrac{\hat\beta_1 - \beta_1}{\widehat{\text{SE}}(\hat\beta_1)} \sim t_{n-k}$.
\vskip6pt
\begin{itemize}
...
\begin{frame}[shrink=10]{Good Controls, Bad Controls}
...
No amount of algebra distinguishes these cases --- the distinction is
\emph{causal}, not statistical. We need a language for causal assumptions:
potential outcomes and DAGs, Week 5.
\end{alertblock}
\end{frame}
