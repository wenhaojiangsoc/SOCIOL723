...
interaction $-0.29$. Take the white, no-record cell as the anchor:
$34\%$ callbacks, so $\hat\beta_0 = \log\frac{0.34}{0.66} = -0.66$.
...
\begin{frame}{Three Readings of $\hat\beta_{\text{record}} = -0.99$, Kept Apart\core}
\begin{center}\scriptsize
\renewcommand{\arraystretch}{1.15}\setlength{\tabcolsep}{4pt}
\begin{tabular}{llll}
\toprule
Scale & Number & Reading & Same for both races? \\
\midrule
log-odds & $-0.99$ & a record subtracts $0.99$ & yes, by construction \\
odds & $e^{-0.99} = 0.37$ & a record multiplies the odds by $0.37$ & yes; $0.28$ with the interaction \\
probability & $0.34 \to 0.16$;\ $0.13 \to 0.04$ & $18$ points (white), $9$ (Black) & \alert{no} \\
\bottomrule
\end{tabular}
\end{center}
\vskip4pt
\begin{itemize}\itemsep3pt
  \item The odds ratio is one number because the model is linear on the
        log-odds scale. The probability change is not one number because
        the S-curve is steep near $0.34$ and flat near $0.13$.
  \item ``$63\%$ lower odds'' is \emph{not} ``$63\%$ fewer callbacks.''
        Callbacks fell by half for whites ($34 \to 17$) and by nearly two
        thirds for Black applicants ($14 \to 5$): risk ratios $0.5$ and
        $0.36$, neither of them $0.37$.
  \item Report the probabilities, as Pager did; the odds ratio at most
        as a footnote.
\end{itemize}
\end{frame}
...
In a logit the effect of $X_1$ on the probability depends on $X_2$
\emph{even with no product term}: the S-curve is steep in the middle and
flat at the ends, and $X_2$ moves people along it.
\begin{itemize}\itemsep3pt
  ...
  \item And leaving the product out imposes one specific, unmotivated form
        of interaction on the probability scale
        \srcp{Berry, DeMeritt, and Esarey 2010}.
  \item \textbf{Practice}: include the product if theory says the latent
        effects interact; then report the interaction you care about as a
        \alert{difference in AMEs}, with an interval:
        \texttt{avg\_comparisons(m, variables = "x1", by = "x2")}.
\end{itemize}
\end{frame}
...
\vskip3pt
\begin{itemize}\itemsep2pt
  \item \textbf{The product, $0.021$}: on the log-odds scale a year of
        parents' education adds $0.197$ for men and $0.197 + 0.021$ for
        women. Not significant ($z = 0.79$, LR $= 0.6$).
  \item \textbf{No-product column}: that model already gives women a
        larger effect on the probability scale, $0.041$ against $0.039$,
        because women sit higher on the S-curve where it is steeper. Ai
        and Norton's point: interaction in probabilities needs no product.
  \item \textbf{With-product column}: the gap is $0.006$ points per year,
        SE $0.004$. That, not $0.021$, answers the question; here the
        answer is no. \texttt{avg\_slopes(m, variables = "pareduc", by = "female")}.
\end{itemize}
\end{frame}
...
\setlength{\abovedisplayskip}{2pt}\setlength{\belowdisplayskip}{2pt}
\begin{align*}
  \loglik = \sum_{i=1}^n \log p_{i,\,y_i}
  \qquad\text{($J = 2$: the Bernoulli log-likelihood).}
\end{align*}
\endgroup
...
\end{itemize}
\vskip2pt
\textbf{The classic.} Breen and Jonsson (2000, \emph{ASR}) modelled
Swedish educational careers as multinomial transitions: ``continue or
...
\begin{itemize}\itemsep2pt
  \item $e^{\hat\beta_j}$ multiplies the expected count, because
        ...
        about a percentage.
  \item \textbf{Exposure}: counts over different windows (years observed,
...
\begin{frame}{Three Repairs, Three Beliefs About the Variance\core}
...
\begin{frame}{Where the Three Numbers Come From\core}
...
\end{itemize}
Lesson: when the three disagree, the disagreement says \emph{where} the
Poisson is wrong.
\end{frame}
...
\begin{itemize}\itemsep2pt
  ...
  \item What no count model fixes: the bump at two children. A norm, not
        a distribution.
\end{itemize}
\end{frame}
...
\end{itemize}
\vskip3pt
\textbf{Overdispersion as the finding. DiPrete, Gelman, McCormick,
Teitler, and Zheng (2011, \emph{AJS}).} ``How many people do you know who
are \dots?'' counts from the 2006 GSS, an overdispersed Poisson per group.
The overdispersion is not a nuisance here: its size measures how unevenly
ties to a group are spread across respondents, that is, how segregated
acquaintanceship is. Verdict: the wide acquaintanceship network is about
as segregated by race as the tiny core network.
\end{frame}
...
\begin{itemize}\itemsep3pt
  \item A zero-inflated negative binomial: one equation for whether a
        person volunteers at all, another for how many hours among those
        who do.
  \item The two equations tell different stories, as in our hurdle: network
        proximity and contact predict \emph{whether}; education predicts
        \emph{how much}, and proximate networks substitute for education.
\end{itemize}
\vskip4pt
\textbf{Where you will and will not see these.} Hurdle and zero-inflated
models are common in health, criminology, and gerontology (hours, arrests,
visits). In \emph{ASR} and \emph{AJS} a negative binomial with robust
standard errors usually carries the day. Reach for a two-part model when
zero is a different kind of answer, not because the zeros are many.
\end{frame}
...
\begin{frame}{Observations Come in Groups (Thursday's Session)\core}
Students in schools, respondents in countries, employees in firms.
\begin{alertblock}{The problem}
Two students in the same school are more alike than two drawn at random.
Their errors are correlated, so $n$ observations carry less than $n$
pieces of information about anything that varies between schools.
\end{alertblock}
\begin{itemize}\itemsep2pt
  \item \textbf{Week 2}: keep OLS, cluster the standard errors. Correct,
        and silent about the groups.
  \item \textbf{Today}: put the group in the model. One new parameter, the
        between-group variance, estimated by maximum likelihood. It
        answers what clustering cannot: how much variation is between
        schools, and do school-level variables matter?
\end{itemize}
\vskip2pt
Running example: High School and Beyond, $7{,}185$ students in $160$
schools ($14$ to $67$ each); outcome a math score (mean $12.6$, SD $6.9$).
\end{frame}
...
\end{align*}
\begin{itemize}\itemsep3pt
  \item $u_j$ is school $j$'s shift of the intercept, shared by all its
        students. It is not estimated as $160$ dummies; it is treated as a
        draw from a distribution with one parameter, $\tau^2$.
  \item Two students in the same school share $u_j$, so
        $\Cov(y_{ij}, y_{i'j}) = \tau^2$ and
        $\Var(y_{ij}) = \tau^2 + \sigma^2$. The model \emph{is} the
        correlation clustering corrects for.
  \item Parameters: $\beta_0, \beta_1, \tau^2, \sigma^2$. Four, whatever
        the number of schools. Fixed effects would need $162$.
\end{itemize}
\vskip3pt
\textbf{Estimation.} Integrate the unobserved $u_j$ out of each school's
likelihood; what remains is a normal likelihood in
$(\bm\beta, \tau^2, \sigma^2)$, maximized by Newton as always.
\texttt{lme4::lmer(y \~{} x + (1 | school))}.
\end{frame}

\begin{frame}{The Intraclass Correlation\core}
Fit the model with no $x$, the \emph{null model}:
\begin{center}\small
\renewcommand{\arraystretch}{1.0}
\begin{tabular}{lc}
\toprule
grand mean $\hat\beta_0$ & $12.64$ \\
between-school variance $\hat\tau^2$ & $8.55$ \\
within-school variance $\hat\sigma^2$ & $39.15$ \\
\bottomrule
\end{tabular}
\end{center}
\begin{align*}
  \text{ICC} = \frac{\tau^2}{\tau^2 + \sigma^2} = \frac{8.55}{8.55 + 39.15} = 0.18 .
\end{align*}
\begin{itemize}\itemsep3pt
  \item $18\%$ of the variance lies between schools, $82\%$ within.
        Equivalently, two classmates' scores correlate at $0.18$.
  \item Is $\tau^2$ zero? LR against plain OLS: $984$.
  \item ICC $= 0$: schools do not matter, OLS is fine. ICC near $1$:
        students are copies of their school.
\end{itemize}
\end{frame}

\begin{frame}{Shrinkage: What the Model Says About One School\core}
The model's estimate of school $j$'s mean is a weighted average of the
school's own mean and the grand mean:
\begin{align*}
...
754--772.
\item DiPrete, Thomas A., Andrew Gelman, Tyler McCormick, Julien Teitler, and
Tian Zheng. 2011. ``Segregation in Social Networks Based on Acquaintanceship
and Trust.'' \textit{American Journal of Sociology} 116(4): 1234--1283.
