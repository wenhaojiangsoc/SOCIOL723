...
One $\bm\beta$ for every cutpoint is an assumption. Check it with four
separate binary logits, ``above cut $j$'' versus not:
\begin{center}\small
\renewcommand{\arraystretch}{1.05}
\begin{tabular}{lcccc}
\toprule
...
\vskip2pt
\begin{itemize}\itemsep1pt
  \item The alternative: the same cumulative model with a separate
        $\bm\beta$ per cut, proportional odds nested by $\bm\beta_1 =
        \cdots = \bm\beta_4$ (not the multinomial, a different family).
  \item Brant test on the table: $32.5$ on $12$ df, $p = 0.001$.
        Vocabulary fails ($12.3$ on $3$ df); the others pass. Its $0.36$
        averages $0.41$ at the first cut and $0.28$ at the rest.
  \item Remedy: free that slope only, partial proportional odds
        (\texttt{VGAM::vglm}, \texttt{ordinal::clm}).
\end{itemize}
\end{frame}
...
\begin{itemize}\itemsep1pt
  ...
  \item \textbf{This week} treats the $u_j$ as draws from a distribution and
        ...
        is between schools? Here $9\%$ once SES and sector are in:
        Coleman et al.'s (1966) question.
\end{itemize}
\end{frame}

\begin{frame}{The Random-Intercept Model\core}
Student $i$ in school $j$, the last frame's equation with distributions:
\begin{align*}
...
\end{align*} \\[-2pt]
\begin{itemize}\itemsep2pt
  \item Each school has its own intercept, $\beta_0 + u_j$: $160$ draws
        from one distribution, not $160$ constants as in fixed effects.
  \item \alert{$\tau^2$ is the between-school variance}: how much school
        intercepts spread around $\beta_0$. \alert{$\sigma^2$ is the
        within-school variance}: how much students spread around their own
        school's line. Total: $\Var(\text{math}_{ij}) = \tau^2 + \sigma^2$.
  \item Classmates share $u_j$, so $\Cov(\text{math}_{ij}, \text{math}_{i'j})
        = \tau^2$: the model \emph{is} the correlation clustering corrects for.
\end{itemize}
\vskip2pt
\textbf{Estimation.} $u_j$ is never observed, so each school's likelihood
averages over its possible values, using the distribution assumed for it;
Newton does the rest.
\texttt{lme4::lmer(math \~{} ses + catholic + (1 | school))}.
\end{frame}

\begin{frame}{Two Assumptions About $u_j$, and Why Estimation Needs Them\core}
The likelihood averages over the unseen $u_j$. Averaging needs a
distribution, and the same one for every school.
\begin{enumerate}\itemsep2pt
  \item \textbf{$u_j \sim \mathcal N(0, \tau^2)$.} The distribution to
        average over. Mean zero is a normalization (any other mean goes
        into $\beta_0$); normal makes the average a normal, so each
        school's likelihood has a closed form with one parameter, $\tau^2$.
  \item \textbf{$u_j$ uncorrelated with SES and Catholic}: $E[u_j \mid X]
        = 0$. The same $\mathcal N(0, \tau^2)$ is used for high-SES and
        low-SES schools. If high-SES schools tended to have higher $u_j$,
        the model would credit that height to SES and $\hat\beta_1$ would
        be too large: an omitted variable, since $u_j$ is part of the
        error. Fixed effects estimate each $u_j$ instead of averaging over
        it; that is what the $160$ dummies buy, and why they lose Catholic.
\end{enumerate}
\end{frame}
...
\begin{itemize}\itemsep1pt
  \item $\text{ICC} = \hat\tau^2/(\hat\tau^2 + \hat\sigma^2) = 3.62/40.6
        = 0.09$: of the variation SES and sector leave unexplained, $9\%$
        is between schools, $91\%$ within. Is $\tau^2$ zero? LR against
        OLS with the same $x$: $332$.
\end{itemize}
\end{frame}
...
\begin{frame}{When You Need a Random Intercept\core}
\begin{enumerate}\itemsep3pt
  \item \textbf{The split itself is the finding.} Tomaskovic-Devey et al.\
        (2020, \emph{PNAS}): earnings in $14$ high-income countries over
        $25$ years, two billion job-years nested in $50$ million
        workplace-years, split into between-workplace and within-workplace
        variance. The between-workplace share rose in $12$ of $14$
        countries and fell in none, fastest in the private sector. That
        share is $\tau^2/(\tau^2 + \sigma^2)$ with the workplace as the
        group; fixed effects and clustered OLS have no such number.
  \item \textbf{A group-level variable with few groups.} Clustered SEs
        are a many-clusters tool: they rely on the clusters being many
        independent draws and are unreliable below a few dozen
        \srcp{Cameron and Miller 2015}. Twelve countries or twenty firms:
        the random intercept's model-based SE is the credible one, at the
        price of assuming $u_j$ normal and uncorrelated with $x$. With
        $160$ schools either works: Catholic $1.94$ ($0.32$) clustered,
        $2.10$ ($0.34$) random intercept.
\end{enumerate}
\vskip4pt
\textbf{Otherwise}: a group-level coefficient, many groups, no interest in
the variance. OLS with clustered SEs is enough.
\end{frame}

\begin{frame}{Random-Intercept Logit, and \texttt{R}\core}
Top quartile in math:
$\Pr(y_{ij} = 1) = \Lambda(\beta_0 + \beta_1\,\text{SES}_{ij} + \beta_2\,\text{Catholic}_j + u_j)$,
$u_j \sim \mathcal N(0, \tau^2)$.
\begin{columns}[c]
\begin{column}{0.50\textwidth}
\centering
\includegraphics[width=\linewidth]{figs/glmer_pred.pdf}
\end{column}
\begin{column}{0.48\textwidth}
\centering\small
\renewcommand{\arraystretch}{0.95}
...
\begin{itemize}\itemsep0pt
  \item Public schools at SES $= 0$: $0.12$ one SD below the average
        school, $0.19$ at $u_j = 0$, $0.29$ one SD above; $0.20$ averaged
        over schools. $\hat\beta_1$ is one school's slope; the average is
        the curve for students overall. Report the one the question asks.
  \item No closed form for the average over $u_j$:
        \texttt{glmer(..., family = binomial, nAGQ = 10)} does it on a grid.
\end{itemize}
\end{frame}
...
\item Cameron, A. Colin and Douglas L. Miller. 2015. ``A Practitioner's Guide
to Cluster-Robust Inference.'' \textit{Journal of Human Resources} 50(2):
317--372.
...
\item Tomaskovic-Devey, Donald, Anthony Rainey, Dustin Avent-Holt, et al.\ 2020.
``Rising Between-Workplace Inequalities in High-Income Countries.''
\textit{Proceedings of the National Academy of Sciences} 117(17): 9277--9283.
