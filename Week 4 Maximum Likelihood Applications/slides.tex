...
Clustering repairs the standard errors and stops there. Two things it leaves
on the table:
\begin{enumerate}\itemsep6pt
  \item \textbf{It has no number for how much variation is between schools.}
        Give the shared part of the error a name, $u_j$, a shift of the
        intercept for school $j$, and its variance is a parameter the
        likelihood estimates. Here $18\%$ of the variance in math scores is
        between schools: the question Coleman et al.\ (1966) made famous,
        and often the finding itself (Sampson, Raudenbush, and Earls 1997).
        Clustered OLS cannot say.
  \item \textbf{It leaves the coefficient where pooled OLS put it.} The SES
        slope is $2.95$ pooled, $2.38$ once schools are in the model,
        $2.19$ within schools. The pooled slope mixes the within-school
        gradient with high-SES students attending high-scoring schools.
        Clustering changes the SE, not the $2.95$.
\end{enumerate}
\vskip4pt
\textbf{Rule.} Groups a nuisance: cluster (Week 2) or fixed effects
(Week 10). Groups part of the question: model them.
\end{frame}
