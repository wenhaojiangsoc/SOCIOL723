...
The likelihood estimated $\tau^2$, not the $160$ shifts. Each $u_j$ comes
afterwards, as a prediction: a weighted average of the school's average
residual from the fixed part, $\bar r_j$, and zero, the average school:
\begin{align*}
  \hat u_j = w_j\,\bar r_j,
  \qquad
...
\begin{tabular}{lcccc}
\toprule
school & $n_j$ & average residual $\bar r_j$ & weight $w_j$ & $\hat u_j$ \\
\midrule
smallest & $14$ & $-7.23$ & $0.58$ & $-4.18$ \\
largest & $67$ & $-1.19$ & $0.87$ & $-1.03$ \\
\bottomrule
\end{tabular}
\end{center}
\begin{itemize}\itemsep1pt
  \item A $14$-student average is noisy, so the model keeps $58\%$ of it and
        pulls the rest toward zero; a $67$-student average keeps $87\%$.
  \item The weight rises with $n_j$ and with $\tau^2/\sigma^2$: the more
        schools differ, the more each school's own evidence counts.
        Fixed effects keep $100\%$, noise included.
...
\vskip3pt
\begin{itemize}\itemsep1pt
  \item \textbf{The assumption that matters}: $u_j$ uncorrelated with SES.
        Here it is false: high-SES schools have higher shocks ($\hat u_j$
        correlate $0.51$ with school-mean SES), and the slope absorbs some
        of that: $2.38$ against the within-school $2.19$.
  \item \textbf{The fix that keeps both}: add school-mean SES as a regressor
        \srcp{Mundlak 1978; Bell and Jones 2015}. SES becomes $2.19$
        ($0.11$), the fixed-effects slope exactly; school-mean SES $3.14$
        ($0.38$) is the between-school part; Catholic $1.23$ ($0.30$).
  \item \textbf{Practice}: within-group causal question, fixed effects.
        Group-level variable or the groups themselves, random intercept
