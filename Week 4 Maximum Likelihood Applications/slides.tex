...
\textbf{Goal}: $\loglik(\bm\beta, \tau^2, \sigma^2)$ when $u_j$ is never
observed. \textbf{Plan}: (1) pretend $u_j$ is known; (2) average over
what $u_j$ could be; (3) multiply over schools. $y_j$: school $j$'s scores.
\begingroup
\setlength{\abovedisplayskip}{2pt}\setlength{\belowdisplayskip}{2pt}
\begin{align*}
  f(y_j \mid u_j)
  &= \prod_{i=1}^{n_j} \phi\big(y_{ij};\ X_{ij}'\bm\beta + u_j,\ \sigma^2\big)
...
\begin{itemize}\itemsep0pt
  ...
  \item Where each parameter lives: $\bm\beta$ in every student's mean;
        $\sigma^2$ in students' spread around their school; $\tau^2$ in
        schools' spread around the grand mean.
\end{itemize}
\end{frame}
...
\begin{align*}
  ...
  \sim \mathcal N\!\left(
  \begin{pmatrix} X_{1j}'\bm\beta \\ X_{2j}'\bm\beta \end{pmatrix},\
