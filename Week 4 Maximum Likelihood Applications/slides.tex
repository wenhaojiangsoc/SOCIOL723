...
Start with no predictors, $\text{math}_{ij} = \beta_0 + u_j + e_{ij}$. The
question is how the raw variation in scores splits between schools and
students, before anything explains it. School $j$'s mean is $\beta_0 + u_j$;
$e_{ij}$ is a student's distance from it, so whatever classmates share is
already in $u_j$.
\begin{columns}[c]
...
between schools $\hat\tau^2$ & $8.55$ \\
within schools $\hat\sigma^2$ & $39.15$ \\
\bottomrule
\end{tabular}
\end{column}
...
\begin{itemize}\itemsep1pt
  ...
  \item With SES and sector in the model, $\tau^2$ and $\sigma^2$ become
        what the controls leave unexplained: $3.62$ and $37.0$. Between
        fell by $58\%$, within by $5\%$: the controls explain differences
        between schools, not between classmates.
\end{itemize}
\end{frame}
