...
OLS puts everything it cannot explain into one error, $e_{ij}$. Split it in
two:
\begingroup
\setlength{\abovedisplayskip}{2pt}\setlength{\belowdisplayskip}{2pt}
\begin{align*}
...
and sector are accounted for, the same number for every student in it.
$e_{ij}$: the student's own deviation. Three things to do with $u_j$:
\begin{itemize}\itemsep2pt
  \item \textbf{Clustered OLS} never separates it. The standard errors are
        repaired; nothing is learned about schools.
  \item \textbf{Fixed effects} (Week 10) estimate every $u_j$: $160$ school
        dummies. That gives the within-school SES slope, but Catholic does
        not vary within a school, so it is absorbed and cannot be estimated.
  \item \textbf{This week} treats the $u_j$ as draws from a distribution and
        estimates their \emph{variance}, one number. Catholic stays
        estimable, and the variance answers what neither of the others can:
        how much of the variation is between schools? Here $18\%$, the
        question Coleman et al.\ (1966) made famous.
\end{itemize}
\end{frame}
