...
\subtitle{Week 9, Regression Discontinuity Designs\\[-6pt]}
...
\begin{frame}{The Idea in One Picture\core}
...
\begin{frame}[allowframebreaks,t]{Setup and Identification\core}
...
\begin{itemize}\itemsep2pt
  ...
  \item \alert{Positivity fails at every $x$, the cutoff included}: assignment is
        deterministic, so $\Pr(D=1\mid X=x)$ is $0$ or $1$ and never strictly
        between. Identification therefore does \emph{not} come from overlap. It
        comes from \textbf{continuity} of $x\mapsto\E[Y_i(d)\mid X_i=x]$, which
        lets the two one-sided limits stand in for the missing counterfactual at
        $c$. That is also why the estimand is local to $c$.
\end{itemize}
\end{frame}

\begin{frame}{The Estimand Is Local, and That Matters\core}
...
\begin{frame}{Estimation: Local Linear Regression\core}
...
\begin{frame}{The Bandwidth Is the Whole Game\core}
\begin{center}
...
\end{itemize}
\end{frame}
\begin{frame}[allowframebreaks,t]{Why the Optimal Bandwidth Breaks the Confidence Interval\adv}
...
\begin{frame}[allowframebreaks,t]{Covariates, Power, and What to Report}
\begin{itemize}\itemsep3pt
...
\begin{frame}{When the Rule Is Not Followed\core}
...
\begin{frame}[allowframebreaks,t]{Fuzzy RD Inherits Everything From IV\core}
\begin{itemize}\itemsep3pt
...
\begin{frame}{The Threat: Manipulation\core}
...
\begin{frame}{Reading and Reporting an RD\core}
...
Problem Set 3 is due Monday October 19. Problem Set 4 will be assigned next
Tuesday (October 27) and is due Monday November 9; it covers Weeks 7 and 9.
\end{block}
\end{frame}
