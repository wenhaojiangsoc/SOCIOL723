...
%% Week 11 -- Synthetic Control and Event Studies
...
\subtitle{\large Week 11 --- Synthetic Control and Event Studies\\[-8pt]}
...
\begin{frame}{When There Is Only One Treated Unit}
...
\begin{itemize}\itemsep3pt
  ...
  \item ``Perhaps the most important innovation in the policy evaluation
...
\begin{frame}{The Optimization Problem, Explicitly}
...
\begin{frame}[shrink=10]{The Factor-Model Justification}
...
\begin{frame}{Why the Constraints Matter}
...
\begin{frame}{Relation to Difference-in-Differences}
...
\begin{itemize}\itemsep3pt
  ...
  \item Synthetic control \emph{searches} for a weighting under which
        ...
        continues.
  \item The formal justification is a linear factor model
        $Y_{jt}(0) = \delta_t + \bm\theta_t'Z_j + \bm\lambda_t'\bm\mu_j + \epsilon_{jt}$:
        if the synthetic control matches the treated unit on a long pre-period,
        it must also match on the unobserved factor loadings $\bm\mu_j$
        \srcp{Abadie, Diamond, and Hainmueller 2010}.
  \item \alert{Length of the pre-period is doing the identification work.} A
        two-period pre-window proves nothing; a twenty-period match is
        informative.
\end{itemize}
\end{frame}

\begin{frame}[shrink=8]{Inference With One Treated Unit}
Conventional standard errors are meaningless here: $n_{\text{treated}} = 1$.
Instead, \alert{permutation inference}.
\vskip3pt
\begin{block}{Placebo (in-space) tests}
Pretend each donor was the treated unit. Estimate its ``effect.'' If the real
treated unit's post-treatment gap is extreme relative to the placebo
distribution, that is evidence.
\end{block}
\vskip3pt
\begin{itemize}\itemsep2pt
  \item Report the ratio of post- to pre-treatment RMSPE for every unit. A large
        post/pre ratio for the treated unit and small ratios elsewhere is the
        result.
  \item The treated unit's rank among the $J+1$ units gives a permutation-style
        $p$-value, with a hard floor of $1/39 \approx 0.026$ for a donor pool of
        38. It is a placebo rank, not an exact test --- exactness would require
        random assignment among these units.
  \item \textbf{Discard poorly fitting placebos} --- a donor the method could not
        match will show a huge gap for reasons unrelated to treatment.
  \item \textbf{In-time placebo}: pretend treatment happened earlier. A ``gap''
        should not appear before the real intervention.
\end{itemize}
\end{frame}
\begin{frame}[shrink=10]{Placebo Inference, Mechanically}
With one treated unit there is no sampling distribution to appeal to. Use
\alert{permutation}: pretend each donor was treated, and compare.
\vskip3pt
...
\begin{frame}[shrink=10]{The Canonical Application: California Proposition 99}
...
\begin{frame}{Diagnostics and Failure Modes}
\begin{itemize}\itemsep3pt
...
\begin{frame}{Reporting a Synthetic Control Study}
...
Article sign-up and confirmation of replication data availability due
\textbf{November 2}.
\end{block}
\end{frame}
