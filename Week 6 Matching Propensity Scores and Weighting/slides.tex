...
\textbf{positivity}, and \textbf{SUTVA},
\begin{align*}
...
$e(x)\approx 0.5$ dominate the regression; cells that are nearly all treated or
nearly all control contribute almost nothing --- to either estimator.
\end{frame}

\begin{frame}[shrink=10]{Two Consequences Worth Stating Plainly}
\begin{itemize}\itemsep4pt
  \item \textbf{Regression and matching answer different questions} even under
        identical assumptions and with the same saturated specification. Neither
        is ``the'' ATE unless effects are homogeneous.
...
\begin{frame}[shrink=10]{Does It Matter Empirically? Angrist (1998)}
...
\end{itemize}
\vskip3pt
\alert{Every one of these is a way of admitting that positivity fails somewhere.}
Being explicit about where is a feature.
\end{frame}

\begin{frame}[shrink=8]{Choices That Actually Matter}
...
have, not a hypothesis about a population; its $p$-value falls mechanically as
matching discards observations \srcp{Imai, King, and Stuart 2008}.}
\end{frame}
...
\begin{frame}[shrink=10]{One Class of Estimands, Three Members}
...
\begin{frame}[shrink=10]{Horvitz--Thompson versus H\'ajek}
...
\begin{frame}[shrink=10]{IPW Is a Weighted Regression}
...
\begin{frame}[shrink=10]{Weighting to Different Estimands}
...
\begin{frame}[shrink=10]{G-Computation: the Other Half}
...
\begin{frame}[shrink=10]{Choosing Among the Methods}
...
\begin{frame}[shrink=10]{A Workflow}
