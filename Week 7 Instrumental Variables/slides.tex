...
\subtitle{Week 7, Instrumental Variables\\[-6pt]}
...
\begin{frame}[allowframebreaks,t]{An Instrument\core}
...
\begin{frame}{The Wald Estimator\core}
...
\begin{frame}{Intuition for the Ratio\core}
...
\begin{frame}[allowframebreaks,t]{Quarter of Birth and Schooling}
...
\end{itemize}
\vskip3pt
{\footnotesize For now, read \emph{compliers} as ``the people whose schooling was
actually moved by their birth quarter,'' and \emph{LATE} as ``the average effect
for those people.'' Later in the lecture we define the four compliance types
properly and show why the IV ratio can only speak for this one group.}
\end{frame}

\begin{frame}[allowframebreaks,t]{The Cautionary Half of Angrist--Krueger}
The paper is also the standard illustration of what goes wrong with many weak
instruments. It previews two objects we formalize in the next two sections:
\textbf{2SLS}, the machinery for combining several instruments, and \textbf{weak
identification}, which explains how adding instruments can make an estimate
worse rather than sharper.
\vskip3pt
\begin{itemize}\itemsep3pt
...
\begin{frame}{The Two Stages\core}
...
\begin{frame}{IV in Matrix Form\adv}
...
\begin{frame}{IV in Matrix Form: The 2SLS Estimator\adv}
...
\begin{align*}
  ...
  = (\bm D'\bm P_Z\bm D)^{-1}\,\bm D'\bm P_Z\,
    \hat{V}(\bm u)\,\bm P_Z\bm D\,(\bm D'\bm P_Z\bm D)^{-1} ,
\end{align*}
plugging in $\hat{V}(\bm u) = \mathrm{diag}(\hat u_i^2)$, and clustering
...
\begin{frame}[allowframebreaks,t]{Four Types of People\core}
...
\begin{frame}{Deriving the LATE: the Denominator\core}
...
\begin{frame}{Living With the LATE\core}
\begin{itemize}\itemsep3pt
...
\begin{frame}{Weak Instruments Are Worse Than No Instrument\core}
Recall $\hat\beta_{IV} = \Cov_n(Z,Y)/\Cov_n(Z,D)$. If the denominator
...
\begin{frame}{Diagnosing Instrument Strength\core}
\begin{itemize}\itemsep3pt
...
\begin{align*}
  C(\beta_0) = \frac{\Big|\En\big[(Y_i - \beta_0 D_i)\,Z_i\big]\Big|^2}
                    {\Var_n\big[(Y_i-\beta_0D_i)Z_i\big]/n}
...
\begin{frame}[allowframebreaks,t]{A Reader's Checklist\core}
...
\begin{frame}[allowframebreaks,t]{When IV Is the Wrong Tool\core}
\begin{itemize}\itemsep3pt
