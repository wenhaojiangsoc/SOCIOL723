...
distribution}. Where it is centred tells you about bias; how wide it is is
...
\end{align*}
with $k$ the number of estimated coefficients (2 in the bivariate case).
\begin{itemize}\itemsep3pt
...
\end{itemize}
\vskip3pt
{\footnotesize (A technicality: the terms share $\bar X$, so first replace
$\bar X$ by $\E[X]$; the difference vanishes as $n$ grows.)}
...
\end{align*}
A $95\%$ confidence interval is $\hat\beta_j \pm t_{0.975,\,n-k}\,
\SE(\hat\beta_j)$ under the exact route, and $\hat\beta_j \pm
1.96\,\SE(\hat\beta_j)$ in the large-sample approximation.
\vskip6pt
\begin{itemize}\itemsep3pt
...
\begin{itemize}\itemsep3pt
  ...
  \item First-order approximation: it can be poor when $g$ is strongly
        nonlinear near $\bhat$. With a ratio whose denominator is near zero,
        the delta method \emph{and} the naive bootstrap can both fail; use a
        ratio-specific interval (Fieller) there.
...
\vskip3pt
If you run $m$ independent tests, all of true nulls, at level $\alpha$, the
chance of at least one false rejection is $1-(1-\alpha)^m$: for $m=20$, about
$64\%$.
\vskip3pt
\begin{itemize}\itemsep2pt
  ...
  \item \textbf{Benjamini--Hochberg}: controls the \emph{false discovery
        rate}, a natural criterion with many outcomes or subgroups when some
...
specifications. Pre-registering a small set of primary hypotheses shrinks
$m$ before any correction is needed; the two are complements, not substitutes.
\end{frame}
...
\begin{itemize}\itemsep3pt
  \item That is the whole of ``heteroskedasticity-robust,'' ``Huber--White,''
        and ``Eicker--Huber--White'' (\texttt{Stata}'s \texttt{robust} adds
        the HC1 scaling on the next frame).
  \item With controls, replace $d_i$ by Week 1's FWL residual $\tilde d_i$
        (the part of the focal regressor the controls cannot predict):
        $\hat V(\hat\beta_j) = \sum_i \tilde d_i^{\,2}\hat e_i^2 \big/
        \big(\sum_i \tilde d_i^{\,2}\big)^2$. Same formula, residualized
        weights.
...
$\E[\hat e_i^2] = \sigma^2(1-h_{ii})$, where $h_{ii}$ is observation $i$'s
\alert{leverage}, in the bivariate case $h_{ii} = \tfrac1n + d_i^2/\sum_j
d_j^2$: large when $X_i$ is unusual.
...
If we knew $\Var(\epsilon_i\mid X_i)=\sigma^2/w_i$, weighted least squares
would restore efficiency: minimize $\sum_i w_i(Y_i - b_0 - b_1X_i)^2$.
\vskip3pt
\begin{itemize}\itemsep3pt
  ...
  \item \alert{Modern practice}: keep OLS, fix the standard errors. Use
        weights with an external justification, sampling weights (to represent
        the population) or known group sizes (a genuine variance argument),
        not a guess about the error variance.
\end{itemize}
\end{frame}
...
\begin{align*}
  Y_{ig} = \beta_0 + \beta_1 X_{ig} + \underbrace{\nu_g}_{\text{cluster shock}}
...
\begin{itemize}\itemsep4pt
  ...
  \item Cluster at the level the sampling or assignment design justifies.
        Never finer than the assignment level; going \emph{coarser} needs its
        own dependence argument, and is not automatically safer.
...
distribution is too thin. As rules of thumb, worry below $G \approx 40$--$50$
and be alarmed below $20$, though performance also depends on cluster-size
imbalance and leverage. A typical symptom: rejection rates of $10$--$15\%$
for a nominal $5\%$ test.
\vskip2pt
...
\begin{enumerate}\itemsep1pt
  ...
  \item \textbf{Aggregate}: collapse to $G$ cluster-level observations and
        analyze those. Honest, but note it reweights and can change the
        estimand.
...
\begin{itemize}\itemsep4pt
  ...
  \item Clustering by state brought rejection rates close to nominal here.
\end{itemize}
\vskip3pt
...
\begin{itemize}\itemsep3pt
  ...
  \item Because it resamples $(Y_i,X_i)$ jointly, the pairs bootstrap is
        ...
        observations; it makes no assumption about $\sigma_i^2$ at all.
...
Residual & $\hat e_i$, then $Y_i^\ast = \hat Y_i + \hat e_i^\ast$
        & requires homoskedasticity; fixed design \\
Wild & $Y_i^\ast = \hat Y_i + \hat e_i v_i$, $v_i \in \{-1,1\}$
...
\begin{itemize}\itemsep4pt
  ...
  \item \alert{\textbf{Bootstrap-$t$ (studentized)}}: bootstrap the \emph{$t$
        ...
        quantiles. Its approximation error shrinks \emph{faster} with $n$
        than the other intervals'\adv. Requires a standard error at each
        replication.
\end{itemize}
\vskip3pt
...
The bootstrap is not magic; the statistic must be \emph{regular} enough (in
a smoothness-like sense; quantiles qualify, the cases below do not).
\begin{itemize}\itemsep2pt
...
Non-normal errors & \textbf{yes} (via large-$n$ approximation) & larger $n$ \\
\midrule
...
\begin{enumerate}\itemsep4pt
  ...
  \item \textbf{Identify the independently sampled or independently assigned
        units, and cluster there.} Use HC2 or HC3 only when observations are
