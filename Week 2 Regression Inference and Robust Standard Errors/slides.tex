...
\begin{frame}{``But $\hat e_i^2$ Is a Terrible Estimate of $\epsilon_i^2$''}
A frequent and reasonable objection: each $\hat e_i^2$ is a single noisy
draw. How can plugging in $n$ bad estimates give a good one?
\vskip4pt
\begin{itemize}\itemsep4pt
  \item Because we never try to learn any individual $\epsilon_i^2$. The
        target is \emph{one number}, the population mean
        $\E[(X_i-\mu_X)^2\epsilon_i^2]$, and
        $\tfrac1n\sum_i d_i^2\,\hat e_i^2$ is a sample average aimed at it:
        the law of large numbers averages the individual noise away.
...
\begin{itemize}\itemsep2pt
  ...
  \item The next frames state the matrix result and then cash it: the
        slope's entry of the matrix sandwich is \emph{exactly} the scalar
        $V$.
\end{itemize}
\end{frame}
...
Two frames ago: each $\hat e_i^2$ is a \emph{noisy} stand-in for
$\epsilon_i^2$, and the meat's average kills the noise. True, but
\alert{averaging only kills noise; a \emph{systematic} error survives the
average} -- and there is one.
\begin{itemize}\itemsep4pt
...
Hold the $X$'s fixed for the bookkeeping. The variance of the error sum then
involves \emph{all} the covariances:
\begin{align*}
  \Var\!\left(\sum_i d_i\epsilon_i \,\Big|\, X\right)
  = \underbrace{\sum_i d_i^2\,\sigma_i^2}_{\text{what HC0 estimates}}
  + \underbrace{\sum_{i}\sum_{j\neq i} d_i d_j\,\Cov(\epsilon_i,\epsilon_j\mid X)}
               _{\text{\alert{assumed zero so far}}} ,
\end{align*}
with $\sigma_i^2 = \Var(\epsilon_i\mid X_i)$, the error spread at $X_i$.
\vskip2pt
\begin{itemize}\itemsep3pt
...
With small $G$, $\hat{V}_{\text{CR}}$ is biased downward, and even the usual
cluster-robust reference distribution ($t$ with $G$-based degrees of freedom,
commonly $G-1$, not $n-k$) is too thin. As rules of thumb, worry below $G \approx 40$--$50$
